        ...
        $('#nextStep').on("click", function(e) {
              // Initialize Complain List 
               ...
               var workspace= $(".workspace");
                ...
                $(workspace).find('input[type="hidden"][name="prescriptionId"]').each(function() {
                     //alert($(this).val());
...
<center>
<h1 style="margin-top:10px;">Prescription Diagnosis</h1>
<input type="hidden" id="visitId" name="visitId" value="${visitId}">
<input type="text" id="prescriptionSearch" name="Search" class="examineSearch" placeholder="Search For Prescription"/>
<img src="<c:url value='/resources/images/add-sign.png'/>" class="addNewButton"/>
</center>
  <div class="signedElements">
      
      <c:forEach items="${signedPrescriptions}" var="prescription">
           <div>
               <input type="hidden" name="prescriptionId" value="${prescription.id}"/>
               <lable class="myLabel">${prescription.name}</lable>
           </div> 
      </c:forEach>    
  </div>
  ...
  <div class="workspace">
    ...
    <div class="unSignedElements">
      <c:forEach items="${unSignedPrescriptions}" var="prescription">
           <div>
               <input type="hidden" name="prescriptionId" value="${prescription.id}"/>
               <lable class="myLabel">${prescription.name}</lable>
           </div> 
