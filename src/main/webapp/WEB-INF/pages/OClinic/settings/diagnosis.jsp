...
<center>
<h1 style="margin-top:10px;">Patient Diagnosis</h1>
<input type="hidden" id="visitId" name="visitId" value="${visitId}">
<input type="text" id="diagnosisSearch" name="Search" class="examineSearch" placeholder="Search For Diagnosis"/>
<img src="<c:url value='/resources/images/add-sign.png'/>" class="addNewButton"/>
</center>
  <div class="signedElements">
      
      <c:forEach items="${signedDiagnosises}" var="diagnosis">
           <div>
  ...
  <div class="workspace">
    ...
    <div class="unSignedElements">
      <c:forEach items="${unSignedDiagnosises}" var="diagnosis">
           <div>
               <input type="hidden" name="diagnosisId" value="${diagnosis.id}"/>
               <lable class="myLabel">${diagnosis.name}</lable>
           </div> 
